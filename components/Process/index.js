...
const Process = () => {
    ...
    return (
        <section id="process" className="py-12 px-4 md:px-8">
                ...
                <div className="relative">
                    ...
                    <div className="space-y-4">  {/* reduced from space-y-8 */}
                        {processSteps.map((step, index) => (
                            <div key={index} className={`lg:w-1/2 ${index % 2 === 0 ? 'lg:pr-16 lg:ml-auto' : 'lg:pl-16'}`}>
                                <div className={`relative p-4 rounded-xl border ${step.color} transition-all duration-300 
                                    hover:shadow-lg hover:-translate-y-1`}>  {/* reduced padding from p-6 to p-4 */}
                                    ...
                                    <div className={`hidden lg:block absolute top-1/2 ${
                                        ...
                                        border-3 border-white -translate-y-1/2`} />  {/* reduced dot size */}
                                    
                                    <div className="flex items-start gap-3">  {/* reduced gap from 4 to 3 */}
                                        <div className={`p-2.5 rounded-lg ${step.color}`}>  {/* reduced padding */}
                                            {step.icon}
                                        </div>
                                        <div className="flex-1">
                                            <div className="flex justify-between items-start mb-1">  {/* reduced margin */}
                                                <h3 className="font-semibold text-slate-800 text-sm">
                                                    {step.title}
                                                </h3>
                                                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${step.color}`}>
                                                    {step.duration}
                                                </span>
                                            </div>
